...
export default async function({sandbox = false} = {}) {
  ...
  //Load octokits
  const api = {graphql: graphqlClient.defaults({headers: {authorization: `token ${token}`}, baseUrl: conf.settings.api?.graphql ?? undefined}), rest: new Octokit({auth: token, baseUrl: conf.settings.api?.rest ?? undefined})}
  ...
  const uapi = session => {
    ...
    if (authenticated.has(session)) {
      ...
      return {login, graphql: graphqlClient.defaults({headers: {authorization: `token ${token}`}}), rest: new Octokit({auth: token})}
    }
    else if (session) {
